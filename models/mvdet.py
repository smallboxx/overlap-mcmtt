...
from improve.overlap import OverLapDecoder, overlab_MaskList
...
class MVDet(nn.Module):
    def __init__(self, Y, Z, X,
        ...
        # )
        if self.is_open_overlap:
            self.overlap_decoder = OverLapDecoder(in_channels=latent_dim,n_classes=2,n_ids=num_ids, Y=self.Y, X=self.X, num_cameras=self.num_cameras)
        else:
    ...
    def forward(self, rgb_cams, pix_T_cams, cams_T_global, vox_util, ref_T_global):
        ...
        if self.mask_list is None and self.is_open_overlap:
            self.mask_list= overlab_MaskList(world_features_, B, S)
        ...
        if self.is_open_overlap:
            out_dict  = self.overlap_decoder(world_features, self.mask_list, feat_cams_)
        else:
            world_features = self.world_feat(world_features.view(B, S * self.feat2d_dim, self.Y, self.X))
